...
if ($user->getStatus() == SUPERVISOR) {
    $get_report = $salary_Process->getReport(PENDING);
} elseif ($user->getStatus() == MANAGER) {
    $get_report = $salary_Process->getReport(ACCEPTED);
}else{
    $get_report = $salary_Process->getAllReports();
}
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <?php while ($row_report = $get_report->fetch_array(MYSQL_ASSOC)) {
                            ...
                            <?php if ($salary_Process->getStatus() == PENDING) : ?>
                            ...
                            <?php elseif ($salary_Process->getStatus() == ACCEPTED) : ?>
                                <td><a href="report.php?approve=<?php echo $salary_Process->getId() ?>"
                                       class="btn btn-xs btn-success">Approve</a></td>
                            <?php endif; ?>
                                </tr>
